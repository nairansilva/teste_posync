...
export class PoTableComponent extends PoTableBaseComponent implements OnInit, AfterViewInit, DoCheck, OnDestroy {
  ...
  @ViewChild('poTableTbody') poTableTbody;
  ...
  showMoreInfiniteScroll(event): void {
    const scrollPosition = event.target.offsetHeight + event.target.scrollTop;
    if (!this.showMoreDisabled && scrollPosition >= event.target.scrollHeight * (this.infiniteScrollDistance / 100)) {
      this.onShowMore();
    }
  }

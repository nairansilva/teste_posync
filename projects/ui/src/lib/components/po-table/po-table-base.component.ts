...
export abstract class PoTableBaseComponent implements OnChanges, OnDestroy {
  ...
  private _infiniteScroll?: boolean;
   ...
   *  - O scroll inifito só funciona para tabelas que utilizam a diretiva p-height.
   *
   * @default `false`
   */
  @Input('p-infinite-scroll') set infiniteScroll(value: boolean) {
    this._infiniteScroll = this.height ? value : false;
  }
   ...
   * @description
   *
   * Define o percentual necessário para disparar o serviço para carregar mais dados na tabela. Caso o valor informado seja maior que 100 ou menor
   * que 0, o valor padrão será 100%
   *
   * **Exemplos:**
   *  - p-infinite-scroll-distance = 80: O sistema quando o sistema atingir 80% do percentual do scroll, o serviço é disparado.
   */
  @Input('p-infinite-scroll-distance') set infiniteScrollDistance(value: number) {
    value > 100 || value < 0 ? (this._infiniteScrollDistance = 100) : (this._infiniteScrollDistance = value);
  }

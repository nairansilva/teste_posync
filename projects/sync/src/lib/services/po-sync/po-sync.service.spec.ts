...
describe('PoSyncService:', () => {
  ...
  describe('Methods:', () => {
    ...
    it('loadData: should load api data', done => {
      poSync['schemas'] = [customerSchema];
      const myDataCustumer = [
        { id: 1, name: 'Customer 1' },
        { id: 2, name: 'Customer 2' }
      ];
      spyOn(poSync, <any>'loadEntityData').and.returnValue(of({ entity: 'Customers', data: myDataCustumer }));
      spyOn(poSync, <any>'groupSchemas').and.returnValue(of({ entity: 'Customers', data: myDataCustumer }));
      poSync.loadData().subscribe(entities => {
        expect(entities[0].entity).toBe('Customers');
        expect(entities[0].data).toEqual(myDataCustumer);
        done();
      });
    });

    it('groupSchemas: deve quebrar os arrays', done => {
      ...
      schemasGroup = poSync['schemasGroup'](teste, 2);

      expect(schemasGroup).toEqual(teste1);
    });

    it('executeForkJoin: deve executar o forkJoin', done => {
      const observable1 = of({ entity: 'Customers' })
      const observable2 = of({ entity: 'login' })
      const groupOfObservables = [observable1, observable2]

      let teste = [0,1,2,3]

      poSync['executeForkJoin'](groupOfObservables, 2);

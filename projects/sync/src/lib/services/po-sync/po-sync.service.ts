...
export class PoSyncService {
   ...
   *
   * @param {number} sizeGroup Quantidade de schemas que serão processados simultaneamente.
   * @returns {Observable<Array<{ entity: string, data: Array<any> }>>} Observable que notificará quando a
   * carga inicial for concluída.
   */
  loadData(sizeGroup = this.schemas.length): Observable<Array<{ entity: string; data: Array<any> }>> {
    const loads: Array<Observable<{ entity: string; data: Array<any> }>> = [];
    let schemasGroup = []

    this.schemas.forEach(el => loads.push(this.loadEntityData(el)));

    schemasGroup = this.groupSchemas(loads, sizeGroup)

    return Observable.create( e=> {
      this.emitter = e;
      this.executeForkJoin(schemasGroup);
    })
  }

    /**
   * Realiza a divisão do array de schemas de acordo com o valor informado pelo usuário no`.
   * poSyncService.loadData
   *
   * @param {Array<Observable<{ entity: string; data: Array<any> }>>} schemasServices Array com todos os schemas cadatrados
   * @param {number} sizeGroup Quantidade de schemas que serão processados simultaneamente.
   * @returns {Array<Observable<{ entity: string; data: Array<any> }>>} Array de Observables contendo os Schemas
   * divididos de acordo com a quantidade informada no poSyncService.loadData.
   */

  private groupSchemas(schemasServices: Array<Observable<{ entity: string; data: Array<any> }>> = [], sizeGroup:number = schemasServices.length): Array<Observable<{ entity: string; data: Array<any> }>> {
    let arrayOfArrays = []

    sizeGroup <= 0 ? sizeGroup = schemasServices.length : sizeGroup = sizeGroup;

    for (let i = 0; i < schemasServices.length; i += sizeGroup) {
      arrayOfArrays.push(schemasServices.slice(i, i + sizeGroup));
    }
    return arrayOfArrays
  }

    /**
   * Realiza a chamada do ForkJoin de maneira recursiva até completar todos os schemas.
   *
   * @param {Array<Observable<{ entity: string; data: Array<any> }>>} forks Array com todos os schemas cadatrados
   * @param {number} count Contador utilizado para controle do laço
   * @param {Array<Object<{ entity: string; data: Array<any> }>>} forkResponse retorno total dos serviços requisitados.
   */
  private executeForkJoin(forks, count = 0, forkResponse = []): void {
    forkJoin(forks[count]).subscribe( reponse => {
      count++
      if (count < forks.length){
        forkResponse = forkResponse.concat(reponse)
        this.executeForkJoin(forks, count, forkResponse)
      } else if ( count === forks.length) {
        forkResponse = forkResponse.concat(reponse)
        this.emitter.next(forkResponse);
      }
    },
    error => {
